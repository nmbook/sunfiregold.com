...
$findbydir = $_GET['findbydir'] ?? '';
if (strlen($findbydir) > 0) {
  // albums/subfolder/index.php directs here
  //$sql =
  //"SELECT `id`
...
if ($id > 0) {
  $row = api_get_album_by_id($DBCONN, $id);
  if ($row['result'] === false) {
    show_message("Album ID '$id' not found.", 'error');
    $id_error = true;
  }
} else {
  $id = 0; // index
...
if ($is_signed_in) {
  ...
  if (isset($image_id)) {
    $image_row = api_get_album_image_by_id($DBCONN, $id, $image_id);
    if ($image_row['result'] === false) {
      show_message("Image ID '$image_id' not found in album.", 'error');
      $act = 10;
    }
  }
  ...
  if ($submit || $act == 4 || $act == 5 || $act == 14 || $act == 15) {
    switch ($act) {
      ...
      case 2: // edit
        ...
        $params = [];
        $params['title'] = $title;
        $params['datetext'] = $datetext;
        if ($act == 1)
        {
          if ($active)
          {
            // visible=true, add to end
            $params['index'] = as_count(api_albums_count($DBCONN, true));
          }
          else
          {
        ...
        $act_descr = ($act == 1 ? 'Added album ' : 'Updated album ')."$title.";

        if ($act == 1)
        {
          $result = api_album_insert($DBCONN, $params, $act_descr);
        }
        else
        {
          $result = api_album_update($DBCONN, $id, $params, $act_descr);
        }
        ...
        exit;
      case 3: // remove
        $act_descr = "Removed album $row[title].";
        
        $dir_location = "./albums/$row[location]/";
        $index_location = $dir_location.'index.php';

        $result = api_album_delete($DBCONN, $row['id'], $act_descr);
      ...
      case 4: // up (index -> inf)
        ...
        if ($old_index == 0)
        {
          show_message("Album $row[title] is hidden and can't be moved.", 'error');
          $act = 0;
          break;
        }
        ...
        $act_descr = "Moved $row[title] album up one position.";
      ...
      case 5: // down (index -> 0)
        ...
        $act_descr = "Moved $row[title] album down one position.";
        ...
        if ($img_count_v) {
          for ($img = 1; $img <= $img_count; $img++) {
            switch ($_FILES["image_$img"]['error']) {
              case UPLOAD_ERR_OK:
                $descs = $_POST["desc_short_$img"] ?? '';
                $descl = $_POST["desc_long_$img"] ?? '';
                $desct = $_POST["desc_time_$img"] ?? '';
                $active = $_POST["active_$img"] ?? 0;
...
<?php
      ...
      $order_by = '`index` ASC';
      echo as_html(api_albums_list($DBCONN, '', '', 1000, 0, '`index` > 0', $order_by));

      if ($is_signed_in)
      {
        $hidden_link_count = as_count(api_albums_list($DBCONN, false));
...
?>
      ...
      <p><a class="edit" href="albums.php?act=add">Create New</a></p>
<?php
          echo as_html(api_albums_list($DBCONN, '', '', 1000, 0, '`index` = 0'));
        }
      }
    } else {
      // view album & images
...
<?php
      echo as_html(api_album_print_navigator($DBCONN, $row['index']));
?>
      </div>
<?php
      $order_by = '`index` ASC';
      $obj = api_album_images_list($DBCONN, '', '', 1000, 0, '`index` > 0', $order_by, $id);
      echo as_html($obj);
      
      if (!array_key_exists('results', $obj) || count($obj['results']) == 0) {
?>
...
?>
      <div id="album_img" class="album_img">
        ...
        <button class="album_img_nav" id="album_img_nav_full">View Full Size</button>
